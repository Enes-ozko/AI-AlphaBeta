...
def alpha_beta_decision(board, turn, ai_level, queue, max_player):
    ...
    for col in moves:
        b2 = board.copy()
        b2.add_disk(col, max_player, update_display=False)
        score = _min_value(b2, depth - 1, alpha, beta, max_player)
    ...
    queue.put(best_move if best_move is not None else moves[0])


def _max_value(board, depth, alpha, beta, max_player):
    ...
    for col in board.get_possible_moves():
        b2 = board.copy()
        b2.add_disk(col, max_player, update_display=False)
        v = max(v, _min_value(b2, depth - 1, alpha, beta, max_player))
        if v >= beta:
            return v 
        alpha = max(alpha, v)
    return v
...
def _min_value(board, depth, alpha, beta, max_player):
    ...
    for col in board.get_possible_moves():
        b2 = board.copy()
        b2.add_disk(col, min_player, update_display=False)
        v = min(v, _max_value(b2, depth - 1, alpha, beta, max_player))
        if v <= alpha:
            return v # élagage 
        beta = min(beta, v)
    return v
...
class Board:
    ...
    def eval(self, player):
        ...
        # Center column bonus
        for j in range(6):
            if self.grid[3][j] == player:
                score += 3
            elif self.grid[3][j] == opponent:
                score -= 3

        # Horizontal
        for y in range(6):
            for x in range(4):
                window = [self.grid[x+i][y] for i in range(4)]
